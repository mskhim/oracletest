...
BEGIN
SUNG_CHANGE_JUM_PROC(:NEW.NO,:NEW.KOR,:NEW.ENG, :NEW.MATH);
COMMIT; 
END;
/
...
END;
/
CREATE OR REPLACE PROCEDURE SUNG_RANK_PROC2
IS
RT SUNG%ROWTYPE;
CURSOR C1 IS
SELECT NO,RANK()OVER(ORDER BY AVG DESC) AS RANK FROM SUNG;
BEGIN
FOR RT IN C1 LOOP
UPDATE SUNG SET RANK = RT.RANK WHERE NO=RT.NO;
END LOOP;
END;
/
...
--등수를 조정해주는 프로시저 실행
EXECUTE SUNG_RANK_PROC2;
--조회
SELECT * FROM SUNG ORDER BY RANK;
