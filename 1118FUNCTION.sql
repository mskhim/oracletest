...
SELECT FIRST_NAME, LAST_NAME, SALARY,ROWNUM FROM(SELECT FIRST_NAME,LAST_NAME,SALARY FROM EMPLOYEES ORDER BY SALARY DESC,FIRST_NAME);
SELECT * FROM (SELECT ROWNUM AS RNUM,E.* FROM EMPLOYEES E) WHERE RNUM BETWEEN 11 AND 20;
SELECT * FROM (SELECT ROWNUM AS RNUM,E.* FROM EMPLOYEES E WHERE ROWNUM<=110) WHERE RNUM >100;